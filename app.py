...
st.markdown(
    ...
    <div style="text-align: center; padding: 20px; background-color: #F0F4FF;
        ...
        <p style="color: #003366; font-size:16px; margin: 0;">
            Загрузите журнал проходов и (по желанию) файл кадров — система автоматически сформирует отчет,
...
st.caption(
    "Контакты нужны только для учёта бесплатных скачиваний и связи по вопросам работы сервиса. Никакой рассылки или передачи данных третьим лицам."
)
            ...
            if free_left_before <= 0:
                ...
                st.markdown(
                    ...
                    ">
                        ...
                        Чтобы получить дополнительный доступ — напишите нам и подключим расширенный режим.
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
...
# Если по какой-то причине final_df не датафрейм — аккуратно завершаем
if not isinstance(final_df, pd.DataFrame):
    st.error(
        "❌ Внутренняя ошибка: отчёт сформирован в неожиданном формате. "
        "Попробуйте ещё раз или отправьте скриншот автору сервиса."
    )
    st.write("Техническая информация:", str(type(final_df)))
    st.stop()
