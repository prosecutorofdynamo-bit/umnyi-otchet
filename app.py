...
st.write("DEBUG_EMAIL:", {
    "EMAIL_USER": repr(EMAIL_USER),
    "EMAIL_PASSWORD_EMPTY": EMAIL_PASSWORD is None or EMAIL_PASSWORD == "",
})
...
st.markdown(
    ...
    <div style="text-align: center; padding: 20px; background-color: #F0F4FF;
        ...
        <p style="color: #003366; font-size:16px; margin: 0;">
            Загрузите журнал проходов и (по желанию) файл кадров — система автоматически сформирует отчёт,
...
st.caption(
    "Контакты нужны только для учёта бесплатных скачиваний и связи по вопросам работы сервиса. "
...
if not isinstance(final_df, pd.DataFrame) or final_df.empty:
    st.error(
        "❌ Внутренняя ошибка: отчёт пустой или в неверном формате. "
        "Попробуйте ещё раз или отправьте ошибку автору сервиса."
    )
