...
if final_df is not None and kadry_dates is not None and not kadry_dates.empty:
    ...
    # 2) Ключи по ФИО:
    #    — берём только первую строку (убираем "(особый режим работы)")
    #    — нормализуем через fio_norm
    def clean_fio_for_key(s: str) -> str:
        s = "" if pd.isna(s) else str(s)
        s = s.split("\n")[0]   # только до перевода строки
        return fio_norm(s)
...
# Базовый набор колонок — ИМЕНА ДОЛЖНЫ СОВПАДАТЬ с тем, что возвращает engine.py
visible_cols = [
    "ФИО",
    "Дата",
    "Время\nприхода",              # как в Колабе
    "Время\nухода",                # как в Колабе
    "Опоздание",
    "Общее время",
    "Вне\nофиса",                  # как в Колабе
